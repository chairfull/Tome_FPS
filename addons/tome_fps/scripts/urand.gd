...
class_name URand
extends RefCounted
## Various static methods for randomness.
...
static func pick_weighted(d: Dictionary, reverse := false, total_weight: Variant = null) -> Variant:
...
# -0.5 - 0.5
static func fbm(x: float = Time.get_ticks_msec() * TIME_SCALE, octaves := 2) -> float:
...
static func fbm_animated(s := 0.0, time_scale := 1.0, octaves := 2) -> float:
	return fbm(s + _time(time_scale), octaves)

static func fbm_animated_lerp(a, b, s := 0.0, time_scale := 1.0, octaves := 2) -> float:
	return lerp(a, b, fbm_animated(s, octaves, time_scale) + .5)

static func fbm_v2(s: Vector2, octaves := 2) -> Vector2:
	return Vector2(fbm(s.x, octaves), fbm(s.y, octaves))

static func fbm_v3(s: Vector3, octaves := 2) -> Vector3:
...
static func _fbm2(x: float, y: float, octave := 2) -> float:
	var total := 0.0
	var amplitude := 0.5
	var maximum := 0.0
	var e := 3.0
...
static func _fbm3(x: float, y: float, z: float, octave:= 2) -> float:
	var total := 0.0
	var amplitude := 0.5
	var maximum := 0.0
	var e := 3.0
